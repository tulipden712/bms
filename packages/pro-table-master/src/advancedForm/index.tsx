...
import { Input, Form, Row, Col, TimePicker, InputNumber, DatePicker, Select, Popover } from 'antd';
...
export interface SearchConfig {
   ...
   * props 更加详细的配置
   * {
      type?: 'advancedform' | 'list' | 'table' | 'cardList' | undefined;
      form: FormInstance;
      submit: () => void;
      collapse: boolean;
...
export interface  FieldLabels<T> {
  name?: T
}
...
export interface TableFormItem<T> extends Omit<FormItemProps, 'children'> {
  ...
  // errors?: { name: string[], errors: React.ReactNode[] }[]
}
...
export const FormInputRender: React.FC<{
  ...
  if(valueType === 'textarea' && type === 'advancedform') {
    return (
      <Input.TextArea
        placeholder={intl.getMessage('tableForm.inputPlaceholder', '请输入')}
        {...rest}
...
const FormSearch = <T, U = {}>({
  ...
  const searchConfig = getDefaultSearch(propsSearch, intl, type === 'advancedform');
  ...
  const getErrorInfo = (errors: { name: string[], errors: React.ReactNode[] }[]) => {
    console.log('getErrorInfo -> errors', errors);
    const errorCount = errors.filter((item: { name: string[], errors: React.ReactNode[] }) => item.errors.length > 0).length;
    ...
    const errorList = errors.map((err: { name: string[], errors: React.ReactNode[] }) => {
      if(!err || err.errors.length === 0) {
        return null;
      }
      const key = err.name[0];
      return (
        <li key={key} className={classNames(`errorListItem`)} onClick={() => scrollToField(key)}>
          <CloseCircleOutlined className={classNames(`errorIcon`)} />
          <div className={classNames(`errorMessage`)}>{err.errors[0]}</div>
          <div className={classNames(`errorField`)}>{fields[key]}</div>
        </li>
      );
    });
    return (
      <span className={classNames(`errorIcon`)}>
        <Popover
          title="表单校验信息"
          content={errorList}
          overlayClassName={classNames(`errorPopover`)}
          trigger="click"
  ...
  const isForm = type === 'advancedform';
  ...
  const columnsList = counter.proColumns
    .filter((item) => {
      ...
      if(item.hideInSearch && type !== 'advancedform') {
        return false;
      }
      if(type === 'advancedform' && item.hideInForm) {
        return false;
      }
      if(
        valueType !== 'index' &&
  ...
  listColConfig = listColConfig.map((_item, idx) => {
    if(((idx - 1) / 3) === 0) {
      return {
        [idx + 1]: { lg: 6, md: 12, sm: 24 }
      }
    }
    if(((idx - 2) / 3) === 0) {
      return {
        [idx + 1]: { xl: { span: 6, offset: 2 }, lg: { span: 8 }, md: { span: 12 }, sm: 24 }
      }
    }
    return {
      [idx + 1]: { xl: { span: 8, offset: 2 }, lg: { span: 10 }, md: { span: 24 }, sm: 24 }
    }
  })
    ...
    ? columnsList
      .map((item, idx) =>
        proFormItemRender({
          ...
          item,
          type,
          colConfig: listColConfig[idx + 1],
          intl,
        }),
      )
      .filter((_, index) => (collapse && type !== 'advancedform' ? index < (rowNumber - 1 || 1) : true))
      .filter((item) => !!item)
    : [];
